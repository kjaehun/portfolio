// Graph class for implementing Dijkstra's algorithm
// 12/09/2021
...
public class Graph<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            this.distance = copyPath.distance; // new distance = original distance + weight of edge being inserted
            this.distance += extendBy.weight;
            this.dataSequence = new LinkedList<>(); // create a deep copy of copyPath's dataSequence
            for (int i=0; i < copyPath.dataSequence.size(); i++) {
                this.dataSequence.add(copyPath.dataSequence.get(i));
            }
            this.dataSequence.add(extendBy.target.data); // append data of target vertex of extendBy
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        PriorityQueue<Path> frontier = new PriorityQueue<Path>(); // holds the discovered paths
        List<Vertex> explored = new ArrayList<>(); // holds the explored vertices (shortest path to these vertices have been found)
        Vertex startVertex = vertices.get(start);
        Vertex endVertex = vertices.get(end); // use the hashtable "vertices" to get the vertices that correspond to the data
        if (startVertex == null || endVertex == null) { // if either of these vertices do not exist in the graph, throw a NoSuchElementException
            throw new NoSuchElementException("Path to target not found");
        }
        ...
        // while loop terminates if frontier is empty or if end vertex has already been explored
        while (frontier.peek() != null && !explored.contains(endVertex)) {
            currentPath = frontier.poll();
            currentVertex = currentPath.end;
            explored.add(currentVertex); // currentPath is shortest path to currentVertex, add currentVertex to explored
            for (Edge e : currentVertex.edgesLeaving) {
                if (!explored.contains(e.target)) { // only add to frontier if target vertex has not been explored yet
                    Path newPath = new Path(currentPath, e); // create new path by adding e to currentPath
                    frontier.add(newPath);
                }
            }
        }
        if (explored.contains(endVertex)) { // if endVertex has been explored (shortest path has been found)
            return currentPath;
        }
        else { // if endVertex has not been explored (path has not been found)
            throw new NoSuchElementException("Path to target not found");
        }
    }
